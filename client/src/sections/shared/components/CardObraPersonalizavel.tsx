...
export default function CardObraPersonalizavel({ aspect, isOpen }: { aspect?: string, isOpen: boolean }) {
            ...
            >
                <Card isFooterBlurred className="group border-none p-0 rounded-xl overflow-hidden">
                    <img
                        ...
                        className={`aspect-${aspect} ${isOpen ? 'group-hover:scale-105 group-hover:brightness-75' : 'brightness-10'}
                    ...
                    <div>
                        {
                            !isOpen &&
                            <div className="w-full h-full top-0 absolute p-4 flex justify-end">
                                <FaLock className="lg:text-[24px] text-[20px] text-gray-300"/>
                            </div>
                        }
                        <CardFooter className={`${theme == "light" ? 'bg-white' : 'bg-black/60 text-gray-200'}
                        ${!isOpen && 'brightness-10'}
                        flex justify-between rounded-lg overflow-hidden p-0.5 absolute before:rounded-xl bottom-1 w-[calc(100%_-_8px)] shadow-small ml-1 z-10`}>
                            ...
                            <div className="flex gap-2 w-full items-center px-2 truncate">
                                ...
                                <p className="text-sm truncate">A luz do Engenheiro</p>
                            </div>
                            <AnimatedGroup>
                                <Link
                                    to="#link"
                                    className="group mx-auto flex w-fit items-center gap-4 rounded-sm border p-1 shadow-md shadow-zinc-950/5 transition-colors duration-300 dark:border-t-white/5 dark:shadow-zinc-950 bg-[var(--secondary-base)] hover:brightness-110"
                                >
